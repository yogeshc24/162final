...
async function connectDatabase() {
    ...
    return new Promise((resolve, reject) => {
        const dbPath = process.env.DB_PATH || 'your_database.db' || 'your_database.db' ;
...
async function startServer() {
    try {
        ...
        await db.get('SELECT * FROM users WHERE hashedGoogleId = ?', [userId]);
...
app.post('/posts', upload.single("image"),(req, res) => {
    ...
    const user = getCurrentUser(req);
    if (user) {
        const imagePath = imagesDir;
        addPost(req.body.title, req.body.content, user, imagePath);
        res.redirect('/');
    } else {
        res.redirect('/login');
    }
...
let users = [
    ...
    },
    {
        ...
        socialMedia: {
            twitter: 'https://twitter.com/EcoWarrior',
            facebook: 'https://facebook.com/EcoWarrior',
...
function addPost(title, content, user, imagePath = null) {
